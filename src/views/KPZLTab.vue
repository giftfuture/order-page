    ...
    >
      ...
      <el-table-column label="状态" align="center" prop="status">
        <template slot-scope="scope" >
          ...
          >
            <el-option
              v-for="item in $store.state.statusDictObj.KP || []"
              :key="item.key"
              :label="item.value"
              :value="item.key"
      ...
      >
      <template slot-scope="scope">
        <el-input  type="textarea" :rows="6" :class="scope.row.deleted === 1?'commonDelete':''" v-model="scope.row.content"  placeholder="请输入开票和资料文本" @blur="handleTextBlur($event,{type: 'content', row:{'id':scope.row.id,'text':scope.row.content} })"/>
      </template>
      </el-table-column>
      <el-table-column
        :show-overflow-tooltip="true"
