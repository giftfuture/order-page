...
export default {
  ...
  methods: {
    ...
    getStatusDict (keys, type) {
      ...
      const data = this.$store.state[type].JH ? this.$store.state[type].JH.filter(item => {
    ...
    search () {
      ...
      this.JHForm.createTimeBegin = this.createTime[0] ? dayjs(this.createTime[0]).format('YYYY-MM-DD HH:mm:ss') : ''
      this.JHForm.createTimeEnd = this.createTime[1] ? dayjs(this.createTime[1]).format('YYYY-MM-DD HH:mm:ss') : ''
      // console.log(this.JHForm.createTimeBegin, 'this.JHForm.createTimeBegin')
      this.handleSearch()
    },
    handleOptions () {
...
<template>
  <div class="commonBody">
    ...
    >
      ...
      <el-table-column label="状态" align="center" prop="status">
        <template slot-scope="scope" >
          <el-select
            ...
            ref='statusRef' >
            <el-option
              v-for="item in $store.state.statusDictObj.JH || []"
              :key="item.key"
              :label="item.value"
              :value="item.key"
      ...
      >
      <template slot-scope="scope">
        <el-input type="textarea" :rows="6" :class="scope.row.deleted === 1?'commonDelete':''" v-model="scope.row.content"  placeholder="请输入进货文本" @blur="handleTextBlur($event,{type: 'content', row:{'id':scope.row.id,'text':scope.row.content} })"/>
      </template>
      </el-table-column>
      <el-table-column
        :show-overflow-tooltip="true"
